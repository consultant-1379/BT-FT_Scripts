...
sub TopoData
{
	...
	if($len ne 0)
	{
	$query="select distinct DATANAME from referencecolumn where typeid like '%$i%' and dataname like '%_FDN' and dataname not like 'SITE_FDN'";
	my @result_from_repdb1= executeSQL("repdb",2641,"dwhrep",$DwhrepPass,$query,"ROW");
	#print Dumper(\@result_from_repdb1);
	$query="select $result_from_repdb1[0] from $i where oss_id like 'eniq_oss_1' and $TopoNodeNamekey like '$Node' ";
	#print "$query\n";
	my @result_from_dwhdb= executeSQL("dwhdb",2640,"dc",$DCPass,$query,"ROW");
	my $len = @result_from_dwhdb;
	if ($len>0)
	{
		return $result_from_dwhdb[0];
	}
	}
}
}
...
sub SNValidation{
	...
	if(int($datetime_hour-2) < 10){
		...
		$time_for_filename=(int($datetime_hour-2))."$datetime_min";}
				...
				$result.=qq{<body bgcolor=GhostWhite> </body> <center> <table  id="$package" BORDER="3" CELLSPACING="2" CELLPADDING="3" WIDTH="100%" >};
				...
				if($len ne 0){
					...
					$query = "select INTERFACENAME from InterfaceTechpacks where TECHPACKNAME LIKE '".$tp_name."'";
					...
					while( my $intf_file = readdir(DIR)) 
					{
						...
						if ($intf_file eq $correct_intf_file) 
						{
							...
							if ($FNODE eq $Nodename)
							{
								...
								my $TopoSN=TopoData($DataFormat,$FNODE,@$r[0]);
								
								$result.=qq{<body bgcolor=GhostWhite> </body> <center> <table  id="$package" BORDER="3" CELLSPACING="2" CELLPADDING="3" WIDTH="100%" >};
								$result .= "<tr><td align=left><font color=black><font size=2px><b>Interface NAME</b></font></font></td><td align=left><font color=orange><font size=2px><b>@$r[0]</b></font></font></td></tr><tr><td align=left><font color=black><font size=2px><b>File SN</b></font></font></td><td align=left><font color=orange><font size=2px><b>$FSN</b></font></font></td></tr>";
								$result_no.=qq{<body bgcolor=GhostWhite> </body> <center> <table  id="$package" BORDER="3" CELLSPACING="2" CELLPADDING="3" WIDTH="100%" >};
								$result_no_node.=qq{<body bgcolor=GhostWhite> </body> <center> <table  id="$package" BORDER="3" CELLSPACING="2" CELLPADDING="3" WIDTH="100%" >};
					
								$result_no.= "<tr><td align=left><font size=3px><font color=magenta><b>NO DATA IN TABLE FOR DATE $datetime:</b></font></font></td></tr>";
								$result_no_node.= "<tr><td align=left><font size=3px><font color=00FF00><b>Tables doesn't have SN or FDN column : $datetime:</b></font></font></td></tr>";
								...
								foreach my $Table ( @Tables ){
									...
									if ( $result_from_dwhdb[0] eq 0 ){
										...
										if($sn_map{$table_name} ne ''){ # node name key check
										
										$query = "select distinct $sn_map{$table_name} from $table_name where OSS_ID like 'eniq_oss_1' and (DATETIME_ID ='$datetimeid_10' OR DATETIME_ID ='$datetimeid_11')";
										#print "Query :$query\n";
										...
										#print "$result_from_dwhdb[0]             $FSN\n";
										if(Contains($FSN,\@DBSN) eq 0){
										
										#print "Fail\n";
										$num++;
										$result .= "<tr><td align=left><font color=000000><b>File FDN compared with Table SN</b></font></td><td align=left><font color=FF0000><b>FAIL</b></font></td></tr>";
										
										}else{
											#print "Pass\n";
											$result .= "<tr><td align=left><font color=000000><b>File FDN compared with Table SN</b></font></td><td align=left><font color=008000><b>PASS</b></font></td></tr>";
											$num1++;
										}
									
										if(Contains($TopoSN,\@DBSN) eq 0){
											#print "FAIL\n";
											$num++;
											$result .= "<tr><td align=left><font color=000000><b>Topology FDN compared with Table SN</b></font></td><td align=left><font color=FF0000><b>FAIL</b></font></td></tr>";
										}else{
										
											#print "PASS\n";
											$result .= "<tr><td align=left><font color=000000><b>Topology FDN compared with Table SN</b></font></td><td align=left><font color=008000><b>PASS</b></font></td></tr>";
											$num1++;
										}
										}else{    # end of if check as 'null'
											$result_no_node.= "<tr><td align=left><font color=000000><b>$table_name</b></font></td></tr>";
										}
									}
								}
								$result .= "</table>";
			...
			{
				$pole = 1;
				print "$package is not having any table with SN Key.\n";
			}
			}
		}
				#################################################
$result.=$result_no."<br>";
$result.=$result_no_node."<br>";
return $result;
}

############################################################################################################################
print "********Executing SN Key Validation Script********\n";
my $stime = getTime();
my $result1=SNValidation();
$report =getHtmlHeader();
$report.="<h1> <font color=MidnightBlue><center> <u> SN Key Data Validation </u> </font> </h1>";
...
if($num1 eq 0 and $num eq 0 and $pole ne 1)
{
	print "SN_Key_Validation:PASS- -1 FAIL- -1\n";
}
else{
print "SN_Key_Validation:PASS- $num1 FAIL- $num No_Data- $empty_num\n";
}
$report.= "</table>";
$report.= "<br>";
...
$report.= getHtmlTail();
my $file = writeHtml("SN_KeyData_Validation_".$datenew,$report);
#print "PARTIAL FILE: $file\n"; 
print "********SN Key Validation Script is done********\n";