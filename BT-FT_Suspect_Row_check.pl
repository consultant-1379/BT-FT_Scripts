...
sub suspect_check{
	...
	if ($tp_name=~ m/(DC_\w*)/){
		foreach my $tp ($tp_name) {
			...
			$result .= "<tr><td align=left><font color=000000><b>TABLE NAME</b></font></td><td align=left><font color=000000><b>Node Name</b></font></td><td align=left><font color=000000><b>No.of Suspect Rows</b></font></td><td align=left><font color=000000><b>RESULT</b></font></td></tr>";
			...
			foreach ( @tp_name_array ){
				...
				$empty_num++;
					...
					my $query3 = "select $Nodenamekey, count(*) from ".$tab_name."_raw where ROWSTATUS like '%SUSPECT%' and DATE_ID ='$date_id' and $Nodenamekey in ('$node_names') group by $Nodenamekey" ;
...
#------------------------------------------------------------------------------------
			...
			if($flag == 0){
				...
				$result .= "<tr><td align=left><font color=FF0000><b>$tab_name</b></font></td><td align=left><font color=FF0000><b>$node_names_output</b></font></td><td align=left><font color=FF0000><b>$counttt</b></font></td><td align=left><font color=FF0000><b>SUSPECT ROW FOUND</b></font></td></tr>";
...
############################################################################################################################
if($suspect_check eq "true")
   {
     print "********Executing Suspect Row Checking Script********\n";

	 
	 my $report =getHtmlHeader();
	$report.="<h1> <font color=MidnightBlue><center> <u> SUSPECT ROW CHECK </u> </font> </h1>";
	...
	my $stime = getTime();
	 ...
	 print "Suspect_Row_Check:PASS- $num1 FAIL- $num No_Data- $empty_num\n";
	 $report.= "</table>";
	 $report.= "<br>";
     ...
     $report.= getHtmlTail();
     my $file = writeHtml("SUSPECT_ROW_CHECK_".$datenew,$report);
	 #print "PARTIAL FILE: $file\n"; 
     $suspect_check ="false";
	 print "********Suspect Row Checking Script Test Case is done********\n";
   }
