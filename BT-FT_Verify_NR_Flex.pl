...
##########################################################################################
# New script to verify the NR flex and flex vectors jira no. EQEV-117180 and EQEV-117178 #
##########################################################################################

my $NR_Flex_check = "true";
my $result;
my $result_no;
my $num;
my $num1;
...
sub Verify_NR_Flex{
	...
	my $file = 'data_new.txt';
		...
		my $csvfile_from_epfg = "/eniq/home/dcuser/epfg/txt/$dir/CountersTxt_".$datetimeepfg."_".$Nodename.".csv";
		my $temptag = '';
		...
		while (<EPFG_FILE>) {
			...
			my $query = "select substr(substr(defaulttags.DATAFORMATID,charindex('))',defaulttags.DATAFORMATID)+3),1,charindex(':',substr(defaulttags.DATAFORMATID,charindex('))',defaulttags.DATAFORMATID)+3))-1) from defaulttags where tagid like '$tagid"."_EVENTS_%' and tagid not like '%DYN%' and DATAFORMATID like '$package%'";
			...
			for my $tablename_repdb (@$result_from_repdb){
				...
				if(@$tablename_repdb[0] =~ m/(.*_V)/){
					...
					my $query_from_measurementcounter = "select DATANAME, COUNTERTYPE from measurementcounter where typeid like '%@$tablename_repdb[0]%'";
					$result_from_repdb = executeSQL("repdb",2641,"dwhrep",$DwhrepPass,$query_from_measurementcounter,"ALL");
					...
					$query = "select $counters,flex_filtername,dcvector_index from @$tablename_repdb[0]_raw where DATETIME_ID = '$datetime' and $Nodenamekey like '$Nodename' and moid like '$moid'";
					...
					#print " ------- $len\n";
						...
						for(my $j=0;$j<$len;$j++){
							...
							if(@$i[$len] eq ''){
								...
								#print "$hash_for_mapping_measurement_counter{$temp[$j]}\n";
								if($hash_for_mapping_measurement_counter{$temp[$j]} eq 'FlexCompVector' and @$i[$j] ne ''){
									push @{$Temp_hash_db_vector{$temp[$j]}}, @$i[$len+1];
									push @{$Temp_hash_db_vector{$temp[$j]}}, @$i[$j];
								}else{
								...
								#print "$hash_for_mapping_measurement_counter{$temp[$j]}\n";
								if($hash_for_mapping_measurement_counter{$temp[$j]} eq 'FlexCompVector' and @$i[$j] ne ''){
									push @{$Temp_hash_db_vector{$t}}, @$i[$len+1];
									push @{$Temp_hash_db_vector{$t}}, @$i[$j];
								}else{
					...
					for my $counter (keys %Temp_hash_db_vector) {
						...
						my $string = join(',',@{$Temp_hash_db_vector{$counter}});
						my($counter1,$flex) = split("_",$counter);
						if($hash_for_mapping_measurement_counter{$counter1} eq 'FlexCompVector'){
							$string = ${$Temp_hash_db_vector{$counter}}[$count-2]+1 .",$string";}
				
							#print "$counter\n";
							if(grep(/^$counter$/,keys %hash_for_mapping_flex_counter)){
								#print "$counter\n";
								if($string eq $hash_for_mapping_flex_counter{$counter}){
									$num++;
									#print "$counter1  pass\n";
									#$result .= "<tr><td align=left><font color=008000><b>$counter1</b></font></td><td align=left><font color=008000><b>$flex</b></font></td><td align=left><font color=008000><b>PASS</b></font></td></tr>";
								}else{
									#print "$counter1\n";
									$num1++;
									#print "$counter  fail\n";
									$result .= "<tr><td align=left><font color=black><b>$counter1</b></font></td><td align=left><font color=black><b>$flex</b></font></td><td align=left><font color=black><b>$moid</b></font></td><td align=left><font color=black><b>FAIL</b></font></td></tr>";
								}
							}
					}
				}else{
							...
							if(@$i[$len] eq ""){
								...
								if(grep(/^$temp[$j]$/,keys %hash_for_mapping_flex_counter)){
									...
									if($hash_for_mapping_flex_counter{$temp[$j]} eq @$i[$j]){
										#print "$temp[$j]    pass\n";
										#$result .= "<tr><td align=left><font color=008000><b>$temp[$j]</b></font></td><td align=left><font color=FF0000><b></b></font></td><td align=left><font color=008000><b>PASS</b></font></td></tr>";
										$num++;
									}else{
										#print "$temp[$j] fail\n";
								...
								if(grep(/^$t$/,keys %hash_for_mapping_flex_counter)){
									if($hash_for_mapping_flex_counter{$t} eq @$i[$j]){
										...
										#$result .= "<tr><td align=left><font color=008000><b>$temp[$j]</b></font></td><td align=left><font color=008000><b>@$i[$len]</b></font></td><td align=left><font color=008000><b>PASS</b></font></td></tr>";
...
if($NR_Flex_check eq "true")
   {
     print "********Executing Verification of NR_Flex Check Script********\n";
	
	my @files = ("ebsn_cucp","ebsn","ebsn_cuup");
	for my $dir (@files){
	$num=0;
	$num1=0;
	my $report =getHtmlHeader();
	$report.="<h1> <font color=MidnightBlue><center> <u> VALIDATION OF NR FLEX AND FLEX VECTORS </u> </font> </h1>";
	...
	my $stime = getTime();
     ...
     $report.= getHtmlTail();
     my $file = writeHtml("VERIFY_TABLE_NR_$dir"."_FLEX".$datenew,$report);
	 #print "PARTIAL FILE: $file\n"; 
     #$dataloadingmo ="false";
	 }
	 print "********Verification of NR_Flex Check  Script Test Case is done********\n";
   }
